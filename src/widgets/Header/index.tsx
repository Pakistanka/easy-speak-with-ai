...
export default function Header({
  ...
  const navConfig = useMemo(() => {
    ...
    const allAuthenticatedNavItems: NavItem[] = [
      ...
      {
        href: '/',
        label: n('becomeAStudent'),
        icon: <BecomeIcon />,
        roles: ['teacher'],
      },
      {
        href: '/',
        label: n('becomeATeacher'),
        icon: <BecomeIcon />,
        roles: ['student'],
      },
    ...
    return allAuthenticatedNavItems.filter(
  ...
  return (
    <>
      <header className="bg-transparent z-50 relative">
        <div
          className={`flex items-center justify-between pt-4 pb-2 px-5 lg:py-10 lg:px-[clamp(3.25rem,calc(-3.5697rem+10.6557vw),6.5rem)] text-[var(--text-color)] text-[clamp(1rem,0.3443rem+1.0246vw,1.3125rem)] ${!withLogo ? 'lg:justify-end' : ''} ${mobileMenuOpen && 'w-full fixed'}`}
        >
          <Logo withLogo={withLogo} />

          {!isLoggedIn && (
            <Navigation items={navConfig} className="hidden lg:flex" />
          )}
